function occurrences(string, subString, allowOverlapping) {
  ...
  while (true) {
    ...
    if (pos >= 0) {
      var character = string.charAt(pos - 1);
      if (character == '\\') {} else ++n;
      pos += step;
    } else break;
  }
...
function filesBetweenDates(start, end, files) {
  if (start.year() != end.year()) {
    for (var x = start.year() + 1; x < end.year(); x++) { //Add Gap Years
      files.push(x);
    }
  ...
  } else if (start.month() != end.month()) { //years match
    for (var x = start.month() + 2; x <= end.month(); x++) { //Add Gap Months
    ...
    fillHours(end, files, true);
    fillDays(end, files, true);
  } else if (start.date() != end.date()) { //years match
    for (var x = start.date() + 1; x < end.date(); x++) { //Add Gap Days
    ...
    fillMinutes(end, files, true);
    fillHours(end, files, true);
  } else if (start.hour() != end.hour()) { //years match
    for (var x = start.hour() + 1; x < end.hour(); x++) { //Add Gap Hours
      files.push(start.format("YYYY/MM/DD/") + (x < 10 ? "0" + x : x));
    }
    fillMinutes(start, files);
    fillMinutes(end, files, true);
  } else if (start.minute() != end.minute()) {
    for (var x = start.minute(); x <= end.minute(); x++) { //Add Gap Minutes
